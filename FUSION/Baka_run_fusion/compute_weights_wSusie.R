## Modified by SSG to include Susie regression
## Need to install 'devtools' R package - conda install -c r r-devtools
#note susieR version
## Then install susieR: devtools::install_github("stephenslab/susieR@0.8.0",build_vignettes = TRUE)
## -------------------------------------------
...
option_list = list(
  ...
  make_option("--plink_mem", action="store", default=1000, type="double", help="Specify maximum amount of memory that PLINK can use"),
  make_option("--plink_threads", action="store", default=1, type="double", help="Specify maximum number of threads that PLINK can use"),
  ...
  make_option("--resid", action="store_true", default=TRUE,
  ...
  make_option("--save_hsq", action="store_true", default=TRUE, ##could set to true?
              help="Save heritability results even if weights are not computed [default: %default]"),			  
  make_option("--models", action="store", default="blup,lasso,top1,enet,susie", type='character',
...
weights.bslmm = function( input , bv_type , snp , out=NA ) {
	...
	return(list(wgt=eff.wgt,fit=eff))
    cat("blup")
}
...
weights.lasso = function( input , hsq , snp , out=NA ) {
	...
	arg = paste( opt$PATH_plink , " --allow-no-sex --bfile " , input , " --lasso " , hsq , " --out " , out , " --memory ", opt$plink_mem, " --threads ", opt$plink_threads, sep='')
	system( arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT )
...
weights.enet = function( genos , pheno , alpha=0.5 ) {
    ...
    cat("enet wgts", head(eff.wgt))
	return( eff.wgt )
}

# Susie
weights.susie = function(genos , pheno) {
	# remove monomorphics
	sds = apply(genos  , 2 , sd)
	keep = sds != 0 & !is.na(sds)
    eff.wgt = matrix(0 , ncol=1 , nrow=ncol(genos[,keep]))
    cat("\n")
    cat(ncol(genos[,keep]))
    cat("\n")
    cat(ncol(genos))
    ...
    cat(pheno)
    #MUST be y not Y!!!
	res = susie(X=genos[,keep], y=pheno, L=10)
	eff.wgt[ keep ] = coef(res)[2:(sum(keep)+1)] # exclude intercept
	res_susie = res
	return(list(wgt= eff.wgt,fit =res_susie))
    cat("susie")
...
if ( !is.na(opt$covar) ) {
    ...
    age_reg = summary(lm( pheno[,3] ~ as.matrix(covar[,ncol(covar)])))
    pheno_reg = reg
    #take a look at reg
...
arg = paste( opt$PATH_plink ," --allow-no-sex --keep-allele-order --bfile ",opt$bfile," --pheno ",pheno.file," --keep ",pheno.file," --make-bed --out ",geno.file , " --memory ", opt$plink_mem, " --threads ", opt$plink_threads, sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
# 1. generate GRM
# using plink is causing some downstream errors for some reason...
#arg = paste( opt$PATH_plink," --allow-no-sex --bfile ",opt$tmp," --make-grm-bin --out ",opt$tmp, " --memory ", opt$plink_mem, " --threads ", opt$plink_threads, sep='')
arg = paste(opt$PATH_gcta, " --bfile ", opt$tmp, " --make-grm --out ", opt$tmp)
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
for ( i in 1:opt$crossval ) {
	...
	arg = paste( opt$PATH_plink ," --allow-no-sex --keep-allele-order --bfile ",opt$tmp," --keep ",cv.file,".keep --out ",cv.file," --make-bed", " --memory ", opt$plink_mem, " --threads ", opt$plink_threads, sep='')
	system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)

	for ( mod in 1:M ) {
		if ( models[mod] == "blup" ) {
            blup_fit=weights.bslmm( cv.file , bv_type=2 , snp=genos$bim[,2] )
			pred.wgt = blup_fit$wgt
		}
            ...
            cat(cv.train[,3])
            cat("\n")
            cat("enet", head(pred.wgt), "\n")
            cat("DIM ENET", dim(pred.wgt), "\n")
		}		
		...
		else if (models[mod]=="susie") {
            susie_fit=weights.susie(genos$bed[ cv.sample[ -indx ],] , as.matrix(cv.train[,3]))
		    pred.wgt = susie_fit$wgt
            cat(cv.train[,3])
            cat("\n")
            cat("DIM SUSIE", dim(pred.wgt), "\n")
...
for ( mod in 1:M ) {
	if ( models[mod] == "blup" ) {
        blup_fit=weights.bslmm( geno.file , bv_type=2 , snp=genos$bim[,2] , out=opt$tmp );
		wgt.matrix[,mod] = blup_fit$wgt
        cat("blup output") 
	}
	else if ( models[mod] == "bslmm" ) {
        ...
        susie_fit=weights.susie( genos$bed , as.matrix(pheno[,3]));
	    wgt.matrix[,mod] = susie_fit$wgt
        cat("susie output")                 
        
	}
}
susiefit=susie_fit$fit
#blupfit=blup_fit$fit                                
                        
# save weights, rsq, p-value for each model, and hsq to output
snps = genos$bim
#save( susiefit, blupfit, wgt.matrix , snps , cv.performance , hsq, hsq.pv, N.tot , pheno_reg, reg, age_reg, file = paste( opt$out , ".wgt.RDat" , sep='' ) )
save( susiefit, wgt.matrix , snps , cv.performance , hsq, hsq.pv, N.tot , pheno_reg, reg, age_reg, file = paste( opt$out , ".wgt.RDat" , sep='' ) )
# --- CLEAN-UP
